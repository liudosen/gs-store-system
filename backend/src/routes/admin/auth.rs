...
async fn store_token(state: &AppState, token: &str, admin_id: u64) -> Result<(), AppError> {
    ...
    for attempt in 1..=2 {
        let mut redis = match state.redis_conn().await {
            Ok(redis) => redis,
            Err(error) => {
                tracing::warn!("Redis pool error storing token: {}", error);
                if state.auth_require_redis_session {
                    return Err(error);
                }
                tracing::warn!(
                    "Redis session store is unavailable; continuing with signed JWT fallback"
                );
                return Ok(());
            }
        };
        ...
        match result {
            ...
            Err(e) if attempt < 2 => {
                tracing::warn!(
                    "Redis error storing token on attempt {}, retrying: {}",
                    attempt,
                    e
                );
...
pub async fn check_token_exists(state: &AppState, token: &str) -> Result<bool, AppError> {
    ...
    for attempt in 1..=2 {
        let mut redis = match state.redis_conn().await {
            Ok(redis) => redis,
            Err(error) => {
                tracing::warn!("Redis pool error checking token: {}", error);
                if state.auth_require_redis_session {
                    return Err(error);
                }
                tracing::warn!(
                    "Redis session check is unavailable; trusting signed JWT for this request"
                );
                return Ok(true);
            }
        };
        ...
        match result {
            ...
            Err(e) if attempt < 2 => {
                tracing::debug!(
                    "Redis error checking token on attempt {}, retrying: {}",
                    attempt,
                    e
                );
                tokio::time::sleep(std::time::Duration::from_millis(100)).await;
            ...
            Err(e) => {
                ...
                if state.auth_require_redis_session {
                    return Err(AppError::InternalError(
                        "Failed to check session".to_string(),
                    ));
                }
                tracing::warn!(
                    "Redis session check is unavailable; trusting signed JWT for this request"
...
async fn query_admin_by_id_with_retry(
    state: &AppState,
    admin_id: u64,
    ...
    for attempt in 1..=2 {
        ...
        match result {
            ...
            Err(error) if attempt < 2 && is_transient_db_error(&error) => {
                tracing::warn!(
                    "Transient database error loading admin by id on attempt {}: {}",
                    attempt,
                    error
                );
...
async fn query_admin_by_username_with_retry(
    state: &AppState,
    username: &str,
) -> Result<Option<AdminUser>, sqlx::Error> {
    for attempt in 1..=2 {
        ...
        match result {
            ...
            Err(error) if attempt < 2 && is_transient_db_error(&error) => {
                tracing::warn!(
                    "Transient database error loading admin by username on attempt {}: {}",
                    attempt,
                    error
                );
                tokio::time::sleep(std::time::Duration::from_millis(150)).await;
...
async fn load_admin_session(state: &AppState, token: &str) -> Result<AdminSession, AppError> {
    ...
    let admin = query_admin_by_id_with_retry(state, claims.admin_id)
        .await?
...
pub async fn login(
    ...
    let admin = query_admin_by_username_with_retry(&state, &body.username)
        .await?
