...
const CATEGORY_SELECT_SQL: &str = "SELECT id, name, sort_order, status, \
    (SELECT COUNT(*) FROM goods WHERE category_id = goods_categories.id) AS goods_count \
...
pub async fn list_categories(
    ...
    let list = sqlx::query_as::<_, GoodsCategory>(&format!(
...
pub async fn create_category(
    ...
    sqlx::query("INSERT INTO goods_categories (name, sort_order, status) VALUES (?, ?, ?)")
        .bind(&name)
        .bind(sort_order)
        .bind(status)
        .execute(&mut *tx)
    ...
    let cat = sqlx::query_as::<_, GoodsCategory>(&format!("{} WHERE id = ?", CATEGORY_SELECT_SQL))
        .bind(id)
        .fetch_one(&state.db)
        .await?;
...
pub async fn update_category(
    ...
    let existing =
        sqlx::query_as::<_, GoodsCategory>(&format!("{} WHERE id = ?", CATEGORY_SELECT_SQL))
            .bind(id)
