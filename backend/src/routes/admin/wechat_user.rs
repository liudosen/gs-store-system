...
fn append_wechat_user_filters(builder: &mut QueryBuilder<MySql>, query: &WechatUserQuery) {
    ...
    if let Some(real_name_status) = query.real_name_status {
...
/// Get user list. Wechat users are the primary table and the default address is attached.
pub async fn list_wechat_users(
    ...
    let mut select_builder = QueryBuilder::<MySql>::new(
        r#"
        SELECT w.id, TRIM(w.openid) AS openid, a.id AS address_id, a.receiver_name,
               a.phone AS address_phone, a.province AS address_province,
               a.city AS address_city, a.district AS address_district,
               a.detail_address, a.label, a.is_default,
