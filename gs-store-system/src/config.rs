    ...
    pub fn from_env() -> Result<Self, env::VarError> {
        Ok(Config {
            ...
            server_port: env_or_default("SERVER_PORT", "8080")
                .parse()
                .unwrap_or(8080),
            database_max_connections: env_u32_or_default("DATABASE_MAX_CONNECTIONS", 20),
            database_min_connections: env_u32_or_default("DATABASE_MIN_CONNECTIONS", 0),
            database_acquire_timeout_seconds: env_u64_or_default(
                "DATABASE_ACQUIRE_TIMEOUT_SECONDS",
                5,
            ),
            database_idle_timeout_seconds: env_u64_or_default("DATABASE_IDLE_TIMEOUT_SECONDS", 60),
            database_max_lifetime_seconds: env_u64_or_default("DATABASE_MAX_LIFETIME_SECONDS", 600),
            auth_require_redis_session: env_bool_or_default("AUTH_REQUIRE_REDIS_SESSION", false),
