...
pub async fn get_dashboard_data(
    ...
    let end_date =
        parse_date(query.end_date.as_deref())?.unwrap_or_else(|| Local::now().date_naive());
    ...
    let completed_orders =
        sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM orders WHERE status = 3")
            .fetch_one(&state.db)
            .await
            .unwrap_or(0);
    let cancelled_orders =
        sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM orders WHERE status = 4")
            .fetch_one(&state.db)
            .await
            .unwrap_or(0);
    ...
    let active_goods = sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM goods WHERE status = 1")
        ...
        .unwrap_or(0);
    let category_count =
        sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM goods_categories WHERE status = 1")
            .fetch_one(&state.db)
            .await
            .unwrap_or(0);
    let low_stock_skus =
        sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM goods_skus WHERE stock_quantity <= 10")
            .fetch_one(&state.db)
            .await
            .unwrap_or(0);
    ...
    .await
    .unwrap_or_default();
    let status_map: HashMap<i8, i64> = status_rows
        .into_iter()
