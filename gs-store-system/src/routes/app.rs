...
use super::admin::{
    ...
    order as admin_order, subscription as admin_subscription, upload, wechat_user,
};
use super::mini_app::{
    address, auth as mini_auth, category as mini_category, goods as mini_goods,
    health_card as mini_health_card, order as mini_order, subscription as mini_subscription,
};
...
async fn health() -> Json<HealthResponse> {
    Json(HealthResponse {
        status: "OK".to_string(),
        service: "gs-store-system-api".to_string(),
    })
}
...
fn admin_routes() -> Router<Arc<AppState>> {
    Router::<Arc<AppState>>::new()
        ...
        .route(
            "/api/admin/goods",
            get(admin_goods::list_goods).post(admin_goods::create_goods),
        )
        .route(
            "/api/admin/goods/{id}",
            get(admin_goods::get_goods)
                .put(admin_goods::update_goods)
                .delete(admin_goods::delete_goods),
        )
        .route(
        ...
        .route(
            ...
            get(wechat_user::list_wechat_users).post(wechat_user::create_wechat_user),
        )
        .route(
...
fn mini_app_routes() -> Router<Arc<AppState>> {
    Router::<Arc<AppState>>::new()
        ...
        .route(
            "/api/mini/userinfo",
            get(mini_auth::get_my_userinfo).put(mini_auth::update_my_userinfo),
        )
        .route("/api/mini/check-id-card", get(mini_auth::check_my_id_card))
        .route(
            "/api/mini/health-card/balance",
            post(mini_health_card::get_my_health_card_balance),
        )
        ...
        .route(
            "/api/mini/orders",
            post(mini_order::create_order).get(mini_order::list_my_orders),
        )
        ...
        .route(
            ...
            post(mini_subscription::set_subscription).get(mini_subscription::get_subscription),
        )
        .route(
        ...
        .route(
            "/api/mini/recharge/quote",
            get(mini_subscription::quote_recharge),
        )
